...
class OrbitSimulation:
    ...
    def setup_initial_conditions(self, dt_start: datetime, v_inf: float) -> np.ndarray:
        ...
        x0 = np.array([
            x_e + 1 * self.config.dict_planet_radius['earth'], 
            y_e, 
            -v_sc_x, 
            v_sc_y
        ])
    ...
    def propagate_orbit_segment(self, 
                               x0: np.ndarray, 
                               travel_days: int, 
                               t_start: int, 
                               dt_start: datetime, 
                               planet_list: List[str]) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        """
        t_span_sec = np.arange(t_start, t_start + travel_days*24*60*60, 24*60*60, dtype=np.int64)
        
        solution = odeint(
            orbital_equation_of_motion_nbody,
            x0, 
            t_span_sec,
            args=(dt_start, planet_list, self.config.dict_GM, self.config.dict_planet_radius)
        )
    ...
    def run_simulation(self,
        ...
        self.planet_coordinates = get_planet_coord_timeseries(self.timeseries, planet_list)
...
def spacecraft_orbit(config=None, OUTPUT_PATH=None, v_inf=None, dt_start=None,
                    travel_days=None, delta_V=None, planet_list=None):
    """
